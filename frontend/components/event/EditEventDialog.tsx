  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updated = await updateEvent(event.event_id, {
        ...
        currency: priceCents ? (event.currency || 'eur') : undefined,
      });
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                placeholder={t('host.events.pricePlaceholder')}
                className="w-full px-3 py-2 border border-border rounded-lg bg-background text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary pr-12"
              />
              <span className="absolute right-3 top-1/2 -translate-y-1/2 text-sm text-muted-foreground">EUR</span>
            </div>
            <p className="text-xs text-muted-foreground mt-1">{t('host.events.priceHint')}</p>
          </div>
