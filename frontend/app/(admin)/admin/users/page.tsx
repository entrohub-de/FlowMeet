...
type Filter = 'all' | 'user' | 'host' | 'admin';
...
export default function AdminUsersPage() {
  ...
  const handleRoleChange = async (userId: string, newRole: UserRole) => {
    const roleLabel = t(`admin.users.role${newRole.charAt(0).toUpperCase() + newRole.slice(1)}`);
    const confirmMsg = t('admin.users.confirmRoleChange', { role: roleLabel });
  ...
  const getRoleIcon = (role: UserRole) => {
    switch (role) {
      ...
      default:
        return <ShieldOff className="w-3 h-3" />;
    }
  };
      ...
      <div className="max-w-4xl mx-auto space-y-4">
        ...
        <div className="bg-card border border-border rounded-xl p-4 flex flex-col sm:flex-row gap-3">
          ...
          <div className="flex gap-2">
            {(['all', 'user', 'host', 'admin'] as Filter[]).map((f) => (
              <button
                key={f}
                onClick={() => setFilter(f)}
        ...
        <div className="text-sm text-muted-foreground">
          {t('admin.users.totalCount', { count: filteredUsers.length })}
        </div>
                ...
                <tbody className="divide-y divide-border">
                  {filteredUsers.map((user) => (
                    <tr key={user.user_id} className="hover:bg-muted/20">
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${getRoleStyle(user.role)}`}
                        >
